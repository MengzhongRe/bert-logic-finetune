...
COUNT_PER_TYPE_PER_DOMAIN = 10  # 单句式单领域生成N条 → 总条数=4句式×6领域×N条
# 示例：5条 → 总120条；10条 → 总240条，反讽每个领域4负1正，其他句式2正2负/2正3负（凑数）

# 情感/保存配置
SENTIMENT_LABELS = ["Positive", "Negative"]
SAVE_DIR = "data"
SAVE_FILENAME = os.path.join(SAVE_DIR, "deepseek_structured_data_cyclic_domain_v2.csv")
...
if __name__ == "__main__":
    ...
    COUNT_PER_TYPE = TOTAL_DOMAIN * COUNT_PER_TYPE_PER_DOMAIN  # 单句式总条数
    total_plan = TOTAL_TYPE * TOTAL_DOMAIN * COUNT_PER_TYPE_PER_DOMAIN
    all_data = []

    # 打印运行信息
    print(f"\n[START] 按「句式×领域」循环生成数据 | 模型: {MODEL_NAME}")
    ...
    print(f"[ADVANTAGE] domain/type硬编码标注，准确率100% | 领域数量绝对均等\n")

    # 双层循环：外层句式，内层领域（tqdm嵌套，展示双进度）
    for t_type in tqdm(TARGET_TYPES, desc="【整体句式进度】", ncols=80, position=0):
        for domain in tqdm(DOMAINS, desc=f"[{t_type}] 领域进度", ncols=80, position=1, leave=False):
            try:
                # 调用API：生成「1个句式+1个领域」的句子
                response = client.chat.completions.create(
                    model=MODEL_NAME,
                    messages=[
                        {"role": "system", "content": system_prompt.strip()},
                        {"role": "user", "content": get_prompt(t_type, domain, COUNT_PER_TYPE_PER_DOMAIN)}
                    ],
                    response_format={"type": "json_object"},
                    temperature=1.3,  # 轻微降低随机性，提升句式贴合度
                    max_tokens=8192,   # 单领域生成数量少，减少token浪费
                    top_p=0.9
                )
                ...
                # 质量校验+硬编码标注
                valid_items = check_data_quality(raw_items, t_type, domain)
                all_data.extend(valid_items)

            except json.JSONDecodeError as e:
                print(f"  [ERROR] [{t_type}-{domain}] JSON解析失败：{str(e)[:80]}")
            except Exception as e:
                print(f"  [ERROR] [{t_type}-{domain}] 生成失败：{str(e)[:80]}")
            continue

    # 最终数据处理与保存（结构化6字段，100%精准）
    if all_data:
        ...
        total_actual = len(df)
        print(f"\n" + "="*70)
        print(f"[FINISH] 数据生成完成！计划{total_plan}条，实际保存{total_actual}条（有效率{total_actual/total_plan*100:.1f}%）")
        ...
        print("\n[ERROR] 未生成任何有效数据，请检查API_KEY、网络或配置")